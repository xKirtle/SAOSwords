...
namespace SAOSwords.UI
{
    ...
    class PersonUI : UIState
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (Main.LocalPlayer.talkNPC == -1 || Main.npc[Main.LocalPlayer.talkNPC].type != SAOSwords.instance.NPCType("Example Person"))
            {
                ...
                SAOSwords.instance.examplePersonUserInterface.SetState(null);
            }
        }
            ...
            if (!vanillaItemSlot.item.IsAir)
            {
                ...
                if (hoveringOverReforgeButton)
                {
                    ...
                    if (Main.mouseLeftRelease && Main.mouseLeft && Main.LocalPlayer.CanBuyItem(awesomePrice, -1) && ItemLoader.PreReforge(vanillaItemSlot.item))
                    {
                        ...
                        {
                            reforgeItem.Prefix(SAOSwords.instance.PrefixType("ReallyAwesome"));
                        }
                        else
                        {
                            reforgeItem.Prefix(SAOSwords.instance.PrefixType("Awesome"));
                        }
