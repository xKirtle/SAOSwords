...
namespace SAOSwords.Items.Quest
{
    public class Anneal : ModItem
    {
        ...
        public class QuestSlime : GlobalItem // Makes so the code below works for all melee weapons
        {
            ...
            private int SlimeKillsLeftUntilAnnealReward = 5; // The little fucker that requires the above methods
            ...
            // Attempt of making the variable of the kills left to get reward on the player rather than on the sword so it works with multiple weapons.
            ...
            /*public override void OnHitNPC(Item item, Player player, NPC target, int damage, float knockBack, bool crit)
            {
                ...
                int bannerID = Item.NPCtoBanner(bSlime);
                // if the target is a Blue Slime, their live goes below zero and the player has the quest's buff
                if (target.type == bSlime && target.life <= 0 && player.GetModPlayer<SAOSwordsPlayer>(mod).testreward >= 2 && player.HasBuff(mod.BuffType("AnnealBuff")))
                {
                    player.GetModPlayer<SAOSwordsPlayer>(mod).testreward -= 1;
                    Main.NewText("You still need to kill more " + player.GetModPlayer<SAOSwordsPlayer>(mod).testreward + " Blue Slimes!", 30, 144, 255);
                }
                // if the target is a Blue Slime, their live goes below zero, the player has the quest's buff and the variable value is equal to 1
                else if (target.type == bSlime && target.life <= 0 && player.HasBuff(mod.BuffType("AnnealBuff")) && player.GetModPlayer<SAOSwordsPlayer>(mod).testreward == 1)
                {
                    int sword = Item.NewItem(target.getRect(), mod.ItemType("AnnealBlade"), 1);
                    NetMessage.SendData(Terraria.ID.MessageID.SyncItem, -1, -1, null, sword); // Drops the reward sword
                    player.ClearBuff(mod.BuffType("AnnealBuff")); // Clears the buff of the quest
            ...
            // Thanks to jopojelly, Ivysaur97 and Dark;Light for the killCount[bannerID] solution (even though I won't use it)
